...
import {
  BulkImportTags,
  UpsertTag,
} from '../../../territory-offline/src/app/core/store/tags/tags.actions';
import { TagSymbol } from '@territory-offline-workspace/shared-interfaces';
import { v4 as uuid4 } from 'uuid';
import {
  UnlockApp,
  UpsertSettings,
} from '../../../territory-offline/src/app/core/store/settings/settings.actions';
import {
  UpsertCongregation,
  UseCongregation,
...
Cypress.Commands.add('createPublishers', (publishers: any[]) => {
  cy.window().then((w) => {
    const p = publishers.map((publisher) => ({
      ...publisher,
      id: uuid4(),
      creationTime: new Date(),
    }));
...
Cypress.Commands.add('configureApp', () => {
  const settings = {
    ...
    // pw: test12345
    passwordHash:
      'liwmju5xwBcudDWizlP2WOkdv5YptVSMU5801wPNHFtyX0FOgyFSdbA+XRpEsKQum0SCNnejX09INoZaR9Ie9g==',
    processingBreakInMonths: 4,
    processingPeriodInMonths: 4,
    publicKey: new Uint8Array([
      212, 89, 113, 202, 106, 187, 14, 157, 177, 141, 109, 212, 102, 145, 143, 57, 121, 21, 224,
      215, 106, 12, 106, 206, 213, 189, 137, 64, 78, 184, 128, 114,
    ]),
    releaseInfo: null,
    secretKey: new Uint8Array([
      232, 170, 162, 176, 27, 249, 201, 36, 31, 250, 104, 134, 27, 253, 120, 26, 179, 147, 195, 111,
      58, 109, 171, 195, 29, 235, 238, 169, 181, 110, 240, 246,
    ]),
  ...
  cy.wait(1);
  cy.window().then((w) =>
    w['store'].dispatch(UseCongregation({ congregationId: congregation.id }))
  );
  cy.wait(1);
