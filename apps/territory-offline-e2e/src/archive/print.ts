describe('InitialeKonfiguration', () => {
  ...
  xit('Screenshot', () => {
    ...
    cy.wait(4000);
    cy.get('[name="layers"] > .feather').click();
    cy.get('.input').type('Stadtbergen');
    cy.wait(1000);
    ...
    cy.get('.main-wrapper > .label')
      ...
      .then((innerText) => {
        ...
        cy.get(
          '.preferences.ng-star-inserted > .wrapper > :nth-child(2) > .main-wrapper > .label'
        ).click();
        cy.get(
          '.preferences.ng-star-inserted > .wrapper > :nth-child(3) > .main-wrapper > .label'
        ).click();

        cy.get('.wrapper > :nth-child(4) > .main-wrapper > .label').click();
        cy.get('.wrapper > :nth-child(5) > .main-wrapper > .label').click();
        cy.exec(
          `screencapture -x -R1240,405,500,355 /Users/lumo/Desktop/Screenshots/Gebiete/${territoryName}.png`
        );
      });
    ...
    for (let i = 2; i < 129; i++) {
      ...
      cy.get('app-list.ng-star-inserted')
        .children()
        .eq(i)
        .then((innerText) => {
          let territoryName = innerText.text();
