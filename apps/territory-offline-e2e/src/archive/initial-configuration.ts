describe('InitialeKonfiguration', () => {
  ...
  it('Beim ersten Start der App muss immer InitialConfiguration geöffnet werden', () => {
    cy.visit('/territories')
      .url()
      .should('contain', 'initial')
      .url()
      .should('not.contain', 'territories');
  });
  it('Eingabe Name-Versammlungsgebiet', () => {
    cy.get('.password-wrapper input')
      .first()
      .clear()
      .type('Augsburg LM')
  ...
  it('Eingabe/Auswahl Sprache', () => {
    ...
    cy.get(languageInput).type('de');
    cy.get('app-language-search app-list-item').first().click();
    cy.get(languageInput).should('have.value', 'Deutsch');
  });
  it('Eingabe Passwort', () => {
  ...
  it('Button Weiter', () => {
    cy.get('.main-button')
      .first() //Klasse gibt immer eine Liste zurück, deswegen Auswahl mit first(), auch wenn es nur einen Button gibt
      .click();
  });
