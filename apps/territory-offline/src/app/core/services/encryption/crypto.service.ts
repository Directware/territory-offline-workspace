...
export class CryptoService
{
  ...
  public isPasswordCorrect(passwordFromUserInput): Observable<boolean>
  {
    return this.store.pipe(
      ...
      first(),
      map(savedPasswordHash => this.tweetNaclService.checkPassword(passwordFromUserInput, savedPasswordHash))
    );
  }
  ...
  public async asymmetricEncryption(dataToBeEncrypted: TimedEntity): Promise<{ id: string, cipher: string }>
  {
    return {
