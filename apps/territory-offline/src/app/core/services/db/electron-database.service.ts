...
export class ElectronDatabaseService implements AbstractDatabase {
  ...
  public async load(
    ...
    const relevantKeys = Object.keys(this.DB_CACHE).filter((key) =>
      key.startsWith(`${congregationPrefix}:${prefix}`)
    );
    const entities: TimedEntity[] = [];

    for (let i = 0; i < relevantKeys.length; i++) {
      const dataStorageResult = await this.database.get({
        key: relevantKeys[i],
      });
      const entity = (await this.cryptoService.asymmetricDecryption(
        dataStorageResult.value
      )) as TimedEntity;
  ...
  public async upsert(
    ...
    const addedEntry = await this.writeToDatabase(
      `${congregationPrefix}:${prefix}`,
      entity
    );
  ...
  public async bulkDelete(
    ...
    for (let i = 0; i < entities.length; i++) {
      await this.database.remove({
        key: `${congregationPrefix}:${prefix}:${entities[i].id}`,
      });
    }
    setTimeout(() => this.cacheStorageDatabaseEntries(), 0);
    return entities;
  ...
  public async delete(
    ...
    await this.database.remove({
      key: `${congregationPrefix}:${prefix}:${entity.id}`,
    });
    setTimeout(() => this.cacheStorageDatabaseEntries(), 0);
    return entity;
  }
