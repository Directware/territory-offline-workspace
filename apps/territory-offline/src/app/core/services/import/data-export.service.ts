...
export class DataExportService
{
  ...
  {
    const currentCongregation = await this.store.pipe(select(selectCurrentCongregation), take(1)).toPromise();
    const today = new Date();
    const minutes = today.getMinutes().toString(10).padStart(2, "0");
    return `${currentCongregation.name} ${today.getFullYear()}-${today.getMonth() + 1}-${today.getDate()} ${today.getHours()}-${minutes}.territoryoffline`;
  }
