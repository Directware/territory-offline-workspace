...
} from '../../store/publishers/publishers.actions';
import {
...
export class DataImportService {
  ...
  public checkCongregationCopy() {
    setTimeout(() => {
      ...
      if (dataPackage) {
        ...
        this.importBackup(parsedData).then(() =>
          localStorage.removeItem(DataImportService.CONGREGATION_COPY_KEY)
        );
      }
    }, 0);
  }
    ...
    if (data && data.type === ExportableTypesEnum.ALL) {
      const translations = await this.translate
        .get([
          ...
          'transfer.import.ok',
          'transfer.import.tags',
          'transfer.import.visitBans',
          'transfer.import.assignments',
          'transfer.import.publishers',
      ...
      const progressMsg = new BehaviorSubject({
        label: translations['transfer.import.start'],
        icon: null,
      });
      ...
      progressMsg.next({
        label: `${filteredImportData.toBeImported.tags.length} ${translations['transfer.import.tags']}`,
        icon: null,
      });
      await this.importTags(
        this.datesFromStringToObject(filteredImportData.toBeImported.tags)
      ).toPromise();
      progressMsg.next({ label: translations['transfer.import.ok'], icon: 'check' });

      const visitBans =
        filteredImportData.toBeImported.visitBans ||
        this.repairTo1VisitBans(filteredImportData.toBeImported.addresses);
      await this.importVisitBans(this.datesFromStringToObject(visitBans)).toPromise();
      progressMsg.next({
        label: `${visitBans.length} ${translations['transfer.import.visitBans']}`,
        icon: null,
      });
      progressMsg.next({ label: translations['transfer.import.ok'], icon: 'check' });
      ...
      progressMsg.next({
        label: `${assignments.length} ${translations['transfer.import.assignments']}`,
        icon: null,
      });
      ...
      const publishers =
        filteredImportData.toBeImported.publisher || filteredImportData.toBeImported.preacher;
      progressMsg.next({
        label: `${publishers.length} ${translations['transfer.import.publishers']}`,
        icon: null,
      });
      ...
      await this.importTerritories(
        this.datesFromStringToObject(filteredImportData.toBeImported.territories)
      ).toPromise();
      progressMsg.next({ label: translations['transfer.import.ok'], icon: 'check' });
  ...
  private repairTo1Drawings(drawings: any[]) {
    const to1Drawings = drawings.filter(
      (drawing) =>
        drawing.featureCollection &&
        drawing.featureCollection.length > 0 &&
      ...
      .forEach((key) => {
        ...
        filteredData[key] = dataToBeImported[key].filter((entityToBeImported) => {
          ...
          if (JSON.stringify(existingEntity) !== JSON.stringify(entityToBeImported)) {
            if (key === 'drawings') {
              if (
                JSON.stringify(this.removePropertiesIfIsDrawing(existingEntity)) !==
                JSON.stringify(this.removePropertiesIfIsDrawing(entityToBeImported))
              ) {
  ...
  private isImportingEntityNewer(
    alreadyExistingEntity: TimedEntity,
    entityToBeImported: TimedEntity
  ) {
    if (
      !alreadyExistingEntity ||
      (!alreadyExistingEntity.lastUpdated && !alreadyExistingEntity.creationTime)
    ) {
      return true;
    }
    ...
    const timeB =
      new Date(entityToBeImported.lastUpdated) || new Date(entityToBeImported.creationTime);
