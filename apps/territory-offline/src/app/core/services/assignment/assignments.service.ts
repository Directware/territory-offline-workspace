...
export class AssignmentsService {
  ...
  public async sendToPublisher(assignment: Assignment) {
    const territory: Territory = await this.store
      .pipe(select(selectTerritoryById, assignment.territoryId), first())
      .toPromise();
    const publisher: Publisher = await this.store
      .pipe(select(selectPublisherById, assignment.publisherId), first())
      .toPromise();
    const drawing = await this.store
      .pipe(select(selectDrawingById, territory.territoryDrawingId), first())
      .toPromise();
    const visitBans = await this.store
      .pipe(select(selectVisitBansByTerritoryId, territory.id), first())
      .toPromise();
    const settings: SettingsState = await this.store
      .pipe(select(selectSettings), first())
      .toPromise();
    const translations = await this.translate
      .get(
        [
          "assignments.digitalTerritory",
          "assignments.sharingFailed",
          "assignments.new",
          "assignments.body",
        ],
    ...
    const gzippedData = Pako.gzip(JSON.stringify(digitalTerritoryCard), {
      to: "string",
    });

    this.platformAgnosticActionsService.share(
      gzippedData,
      `${territory.key} ${territory.name}.territory`,
      "territory-cards"
    );
  ...
  public async giveBackFromFieldCompanion(territoryCard: TerritoryCard) {
    const translation = await this.translate
      .get("assignments.returnFromFieldCompanion", {
    ...
    this.territoryMapsService.focusOnDrawingIds(
      [territoryCard.territory.territoryDrawingId],
      async () => {
        const resp = confirm(translation);
        if (resp) {
          const assignments = await this.store
            .pipe(select(selectLastAssignmentOfEachTerritory), first())
            .toPromise();
          const assignment = assignments.filter(
            (a: Assignment) => a.territoryId === territoryCard.territory.id
          )[0];
