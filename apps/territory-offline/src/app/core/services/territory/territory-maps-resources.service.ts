...
export class TerritoryMapsResourcesService {
  ...
  public focusOnMarkers(padding) {
    const tmp = [];
    Object.values(this.currentMarkers).forEach((marker: any) =>
      tmp.push([marker.getLngLat().lng, marker.getLngLat().lat])
    );
    if (tmp.length > 1) {
  ...
  public initDrawMode(
    ...
    this.map.on("draw.create", (e) =>
      callback ? callback(e, this.drawManager) : null
    );
    ...
    this.drawManager.add({
      ...
      features: initialFeatureCollection.features.filter(
  ...
  public setDataForSourceId(
    ...
    if (currentSource) {
      ...
      console.log(
        `[TerritoryMapsResourcesService - setDataForSourceId]: No source found! (sourceId=${sourceId})`
      );
    }
  }
