...
export class GeoJsonParseService {
  ...
  public readGEOJson(geoJson: any, propMappings: any = {}): { t: Territory; d: Drawing }[] {
    ...
    this.onlyPolygons(geoJson).forEach((feature, index) => {
      ...
      const idx = index + 1;
      territory.name = propMappings.name
        ? feature.properties[propMappings.name]
        : `Territory ${idx}`;
      territory.key = propMappings.key
        ? feature.properties[propMappings.key]
        : `${idx.toString(10).padStart(3, '0')}`;
      territory.populationCount = propMappings.populationCount
        ? feature.properties[propMappings.populationCount]
        : 0;
