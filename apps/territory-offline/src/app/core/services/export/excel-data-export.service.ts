        ...
        this.store
          .pipe(
            ...
            tap((publishers) => {
              ...
              publishers.forEach(
                (p, index) =>
                  (tmp[index + 1] = [p.firstName, p.name, p.email || '-', p.phone || '-'])
              );
        ...
        this.store
          .pipe(
            ...
            tap((territories) => {
              ...
              const tmp = [
                [translations['transfer.export.number'], translations['transfer.export.name']],
              ];
...
* */
  public async exportTerritoryState() {
    ...
    const countOfAllNotProcessed = assignments
      ...
      .filter(
        (endTime) => today.diff(endTime, 'months') > settings.processingPeriodInMonths
      ).length;
  ...
  private tmp(visitBan: VisitBan) {
    ...
    const latitude =
      visitBan.gpsPosition && visitBan.gpsPosition.lat ? `${visitBan.gpsPosition.lat}` : '';
    const longitude =
      visitBan.gpsPosition && visitBan.gpsPosition.lng ? `${visitBan.gpsPosition.lng}` : '';
    return [
      name,
      floor,
      visitBan.street,
  ...
  private async saveWorkBook(workBook: any, fileName: string) {
    ...
    await this.platformAgnosticActionsService.share(
      wbout,
      `${fileName} - ${today.getDate()}-${today.getMonth() + 1}-${today.getFullYear()}.xlsx`
    );
  }
}
