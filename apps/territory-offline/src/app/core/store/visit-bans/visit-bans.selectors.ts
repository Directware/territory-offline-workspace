...
export const selectAllVisitBanEntities = createSelector(
  selectEntities,
  (visitBans) => visitBans
);

export const selectAllVisitBans = createSelector(
  selectAll,
  (visitBans) => visitBans
);
...
export const selectVisitBansCount = createSelector(
  selectTotal,
  (total) => "" + total
);

export const selectVisitBansByTerritoryId = createSelector(
  selectAll,
  (visitBans, id) => visitBans
...
export const selectOverdueVisitBans = createSelector(
  selectAll,
  (visitBans) =>
  {
    const overdue = visitBans.filter(vb => !vb.lastVisit || vb.lastVisit < pastDateByMonths(12)).sort((vb1, vb2) => vb1.street > vb2.street ? 1 : -1);
