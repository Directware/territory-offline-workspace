...
const visitBansReducer = createReducer(
  ...
  on(BulkImportVisitBansSuccess, (state, action) =>
    visitBansAdapter.upsertMany(action.visitBans, state)
  ),
  on(DeleteVisitBanSuccess, (state, action) =>
    visitBansAdapter.removeOne(action.visitBan.id, state)
  )
);
