...
} from '../assignments/assignments.actions';
...
export class AssignmentsEffects {
  ...
  private deleteAssignmentsByTerritory$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((action) =>
        of(action).pipe(
          withLatestFrom(
            this.store.pipe(select(selectAssignmentsByTerritoryId, action.territoryId))
          )
        )
      ),
      map(([action, assignments]: [any, any]) =>
        this.database.bulkDelete(HASHED_ASSIGNMENT_TABLE_NAME, assignments)
      ),
      switchMap((promise: Promise<TimedEntity[]>) => from(promise)),
      map((assignments: Assignment[]) =>
        DeleteAssignmentsByTerritorySuccess({ assignments: assignments })
      )
    )
  );
  ...
  public onUpsertAssignmentSuccess = createEffect(() =>
    this.actions$.pipe(
      ofType(UpsertAssignmentSuccess),
      concatMap((action) =>
        of(action).pipe(
          withLatestFrom(
            this.store.pipe(select(selectTerritoryById, action.assignment.territoryId))
          )
        )
      ),
      map(([action, territory]) => {
        const lastDoingAction = action.assignment.endTime
          ? LastDoingActionsEnum.ASSIGN_RETURN
          : LastDoingActionsEnum.ASSIGN;
  ...
  public onDeleteAssignmentSuccess = createEffect(() =>
    this.actions$.pipe(
      ofType(DeleteAssignmentSuccess),
      map(() => UpdateStatusOfDrawings())
    )
  );
