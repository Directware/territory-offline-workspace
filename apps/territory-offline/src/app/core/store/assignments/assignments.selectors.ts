...
import {
  ...
  currentServiceYear,
  includedInThisServiceYear,
  pastDateByMonths,
} from "@territory-offline-workspace/shared-utils";

export const selectAssignmentsFeature = (state: ApplicationState) =>
  state.assignments;
  ...
  (lastAssignments, territories, settings) => {
    ...
    const currentlyDonePopulationCount =
      territories && territories.length > 0
        ? territories
            .filter((t) =>
              currentlyDone.map((a) => a.territoryId).includes(t.id)
            )
            .map((t) => t.populationCount)
            .reduce((total, current) => total + current, 0)
        : 0;
    ...
    return {
      ...
      doneInThisServiceYear: doneInThisServiceYear.length,
...
export const selectOverdueAssignments = createSelector(
  ...
  (lastAssignments, settings, publisherEntities) => {
    ...
    currentlyInProgress
      .filter(
        (a) => a.startTime < pastDateByMonths(settings.processingPeriodInMonths)
      )
...
export const selectOverdueAssignmentsByPreacher = createSelector(
  ...
  (dtos, publisherId) => {
    const filteredDto = dtos.filter(
      (dto) => dto.publisher.id === publisherId
    )[0];
...
export const selectOverdueTerritories = createSelector(
  ...
  (
    ...
    const overdue = [];
    territories.forEach((t) => {
      const lastAssignment = lastAssignments.filter(
        (a) => a.territoryId === t.id
      )[0];
      ...
      if (
        ...
        (!!lastAssignment.endTime &&
          lastAssignment.endTime <
            pastDateByMonths(settings.overdueBreakInMonths))
      ) {
        overdue.push({
          territory: t,
          lastAssignmentEndTime: lastAssignment?.endTime,
          durationPhrase: createDurationPhrase(
            lastAssignment ? lastAssignment.endTime : null
          ),
        });
      }
    });
  ...
  (assignments: Assignment[], territories, publisher, tagState) => {
    ...
    Array.from(assignmentsByTerritory.values()).forEach((dto) =>
      dto.assignmentDtos.sort((dto1, dto2) => {
        ...
        return dto1.assignment.startTime.getTime() >
          dto2.assignment.startTime.getTime()
          ? -1
          : 1;
      })
    );
    ...
    territories
      ...
      .forEach((territory) =>
        helperSet.add({
          t: territory,
          durationPhrase: createDurationPhrase(
            currentAssignments.filter((a) => a.territoryId === territory.id)[0]
              ?.startTime
          ),
        })
      );
