...
} from '../publishers/publishers.selectors';
...
export const selectAllAssignments = createSelector(selectAll, (assignments) => assignments);
...
export const selectAssignmentsByPublisherId = createSelector(selectAll, (assignments, id) =>
  assignments.filter((a) => a.publisherId === id)
);

export const selectAssignmentsByTerritoryId = createSelector(selectAll, (assignments, id) =>
  assignments
    .filter((a) => a.territoryId === id)
...
export const selectCurrentlyOpenAssignments = createSelector(selectAll, (assignments) =>
...
export const selectLastEndedAssignmentOfEachTerritory = createSelector(
  ...
  (assignments: Assignment[], territories) => {
    ...
    territories.forEach((t) => {
      const lastAssignment = assignments
        .filter((a) => a.territoryId === t.id)
        .sort((a1, a2) => (a1.endTime > a2.endTime ? -1 : 1))[0];
      lastAssignments.push(lastAssignment || {});
    });
...
export const selectDashboardData = createSelector(
  ...
  (lastAssignments, territories, settings) => {
    const currentlyDone = lastAssignments.filter(
      (a) => a && !!a.startTime && !!a.endTime && a.endTime > pastDateByMonths(12)
    );
    const currentlyInProgress = lastAssignments.filter((a) => a && !!a.startTime && !a.endTime);
    const totalPopulationCount =
      territories && territories.length > 0
        ? territories.map((t) => t.populationCount).reduce((total, current) => total + current, 0)
        : 0;

    const currentlyDonePopulationCount =
      territories && territories.length > 0
        ? territories
            .filter((t) => currentlyDone.map((a) => a.territoryId).includes(t.id))
            .map((t) => t.populationCount)
...
export const selectOverdueAssignments = createSelector(
  ...
  (lastAssignments, settings, publisherEntities) => {
    ...
    currentlyInProgress
      .filter((a) => a.startTime < pastDateByMonths(settings.processingPeriodInMonths))
...
export const selectOverdueTerritories = createSelector(
  ...
  (territories: Territory[], lastAssignments: Assignment[], settings: SettingsState) => {
    ...
    territories.forEach((t) => {
      ...
      if (
        !lastAssignment ||
        (!!lastAssignment.endTime &&
          lastAssignment.endTime < pastDateByMonths(settings.overdueBreakInMonths))
      ) {
        overdue.push({
          territory: t,
...
export const selectAllAssignmentsOrderedByRelevantTags = createSelector(
  ...
  (assignments: Assignment[], territories, publisher, tagState) => {
    ...
    territories
      .filter((territory) => !territory.deactivated)
      .forEach((territory: Territory) => {
        assignmentsByTerritory.set(territory.id, {
          territory: territory,
          showCount: 4,
          initialShowCount: 4,
          assignmentDtos: assignments
    ...
    Array.from(dtosByTag.values()).forEach((dto) =>
      dto.territoryDtos.sort((d1, d2) =>
        parseInt(d1.territory.key, 10) < parseInt(d2.territory.key, 10) ? -1 : 1
      )
    );
    ...
    return (
      Array.from(dtosByTag.values())
        ...
        .sort((dto1, dto2) =>
          parseInt(dto1.territoryDtos[0].territory.key, 10) <
          parseInt(dto2.territoryDtos[0].territory.key, 10)
            ? -1
            : 1
        )
    );
...
export const selectTerritoriesByPublisher = createSelector(
  ...
  (
    assignments,
    territories,
    lastEndedAssignments,
    publisherId
  ): { t: Territory; durationPhrase: string }[] => {
    const currentAssignments = assignments.filter(
      (a) => a.publisherId === publisherId && !a.endTime
    );
    ...
    territories
      .filter((t) => publishersTerritoryIds.includes(t.id))
      .forEach((territory) =>
        helperSet.add({
          t: territory,
          durationPhrase: createDurationPhrase(
            currentAssignments.filter((a) => a.territoryId === territory.id)[0]?.startTime
          ),
        })
      );
