...
const assignmentsReducer = createReducer(
  ...
  on(DeleteAssignmentSuccess, (state, action) => assignmentsAdapter.removeOne(action.assignment.id, state)),
  on(DeleteAssignmentsByTerritorySuccess, (state, action) => assignmentsAdapter.removeMany(action.assignments.map(a => a.id), state))
);
