...
export class SettingsEffects {
  public loadSettings$ = createEffect(() =>
    this.actions$.pipe(
      ofType(LoadSettings),
      tap(() => console.log("[SettingsEffects] side effect: LoadSettings")),
      map((action) => this.database.load()),
      switchMap((promise: Promise<SettingsState>) => from(promise)),
      tap((settings) =>
        console.log("[SettingsEffects] loaded settings:", settings)
      ),
  ...
  public lockApp$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(LockApp),
        tap(() =>
          this.router.navigate([{ outlets: { global: ["lock-screen"] } }])
        )
      ),
    { dispatch: false }
  );
