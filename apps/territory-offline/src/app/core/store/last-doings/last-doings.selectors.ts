...
export const selectLastDoings = createSelector(selectAll, (lastDoings) =>
  lastDoings.sort((ld1, ld2) => (ld1.creationTime.getTime() < ld2.creationTime.getTime() ? 1 : -1))
);

export const selectLastDoingsForTidyUp = createSelector(selectAll, (lastDoings) =>
  lastDoings.sort((ld1, ld2) => (ld1.creationTime.getTime() < ld2.creationTime.getTime() ? 1 : -1))
);
