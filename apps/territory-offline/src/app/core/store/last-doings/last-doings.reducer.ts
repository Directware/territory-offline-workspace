...
const lastDoingsReducer = createReducer(
  ...
  on(UpsertLastDoingSuccess, (state, action) =>
    lastDoingsAdapter.upsertOne(action.lastDoing, state)
  ),
  on(DeleteLastDoingSuccess, (state, action) =>
    lastDoingsAdapter.removeOne(action.lastDoing.id, state)
  )
);
