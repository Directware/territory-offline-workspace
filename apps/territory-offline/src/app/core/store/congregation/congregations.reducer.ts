...
const congregationsReducer = createReducer(
  initialState,
  on(LoadCongregationsSuccess, (state, action) =>
    congregationsAdapter.addMany(action.congregations, state)
  ),
  on(UpsertCongregationSuccess, (state, action) =>
    congregationsAdapter.upsertOne(action.congregation, state)
  ),
  on(BulkImportCongregationsSuccess, (state, action) =>
    congregationsAdapter.upsertMany(action.congregations, state)
  ),
  on(DeleteCongregationSuccess, (state, action) =>
    congregationsAdapter.removeOne(action.congregationId, state)
  ),
...
export function congregationsReducerFunction(
  state: CongregationsState | undefined,
  action: Action
) {
