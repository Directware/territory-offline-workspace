...
export class CongregationsEffects {
  private loadCongregations$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((congregations: Congregation[]) =>
        LoadCongregationsSuccess({ congregations: congregations })
      )
    )
  );
  ...
  private upsertCongregation$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((action) =>
        this.database.upsert(HASHED_CONGREGATION_TABLE_NAME, action.congregation, true)
      ),
      switchMap((promise: Promise<TimedEntity>) => from(promise)),
    ...
    () =>
      this.actions$.pipe(
        ...
        concatMap((action) =>
          of(action).pipe(
            withLatestFrom(
              this.store.pipe(select(selectSettings)),
              this.store.pipe(select(selectCurrentCongregation))
            )
          )
        ),
        tap(([action, settings, congregation]) => {
          ...
          if (action.congregationId !== settings.currentCongregationId) {
            ...
            this.store.dispatch(
              UpsertSettings({
                settings: { ...settings, currentCongregationId: action.congregationId },
              })
            );
          }
        })
  ...
  private deleteCongregation$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((action) =>
        this.database.delete(HASHED_CONGREGATION_TABLE_NAME, action.congregation, true)
      ),
      switchMap((promise: Promise<TimedEntity>) => from(promise)),
      tap((congregation: Congregation) =>
        this.lastDoingsService.createLastDoing(LastDoingActionsEnum.DELETE, congregation.name)
      ),
