...
const publishersReducer = createReducer(
  ...
  on(UpsertPublisherSuccess, (state, action) =>
    publishersAdapter.upsertOne(action.publisher, state)
  ),
  on(BulkUpsertPublisherSuccess, (state, action) =>
    publishersAdapter.upsertMany(action.publisher, state)
  ),
  on(BulkImportPublishersSuccess, (state, action) =>
    publishersAdapter.upsertMany(action.publishers, state)
  ),
  on(DeletePublisherSuccess, (state, action) =>
    publishersAdapter.removeOne(action.publisher.id, state)
  )
);
