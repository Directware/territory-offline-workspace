...
export class SearchTerritoryPipe implements PipeTransform {
  ...
  public transform(
    ...
    search: { value: string },
    filteredDrawings: { [id: string]: boolean }
    ...
    return this.store.pipe(
      ...
      tap((territories: Territory[]) =>
        this.mapsService.focusOnDrawingIds(territories.map((t) => t.territoryDrawingId))
      ),
      tap((territories: Territory[]) => {
        if (territories.length === 1 && !!territories[0] && !!territories[0].id) {
          this.router.navigate([
            { outlets: { 'second-thread': ['territory', territories[0].id] } },
          ]);
        }
      })
    );
  ...
  private considerFilteredDrawings(
    territory: Territory,
    filteredDrawings: { [id: string]: boolean }
  ) {
