...
export class SearchPublisherPipe implements PipeTransform
{
  ...
  public transform(publishersByLetters: Publisher[][], search: {value: string}): Observable<Publisher[][]>
  {
    ...
    return this.store
      .pipe(
        select(selectTags),
        map((tags: Tag[]) => publishersByLetters
          .filter(pbl => pbl.filter(p => this.filterPublisher(p, tags, search.value.toLowerCase())).length > 0)
          .map(pbl => pbl.filter(p => this.filterPublisher(p, tags, search.value.toLowerCase()))))
      );
  }
