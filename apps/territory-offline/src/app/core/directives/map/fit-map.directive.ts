...
export class FitMap implements AfterViewChecked {
  ...
  constructor(private territoryMapsService: TerritoryMapsService, private elementRef: ElementRef) {}
  ...
  private considerFitMap() {
    if (this.menu && this.mainThread && this.secondThread) {
      ...
      if (
        this.lastState !== 50 &&
        menuDim.left === 0 &&
        ((mainThreadDim.left > 0 && secondThreadDim.left < 0) ||
          mainThreadDim.left === secondThreadDim.left)
      ) {
        this.territoryMapsService.setPadding({
          top: pad,
          right: pad,
          bottom: pad,
          left: menuDim.width + mainThreadDim.width + pad,
        });
        this.lastState = 50;
        return;
      }
      ...
      if (
        this.lastState !== 60 &&
        menuDim.left === 0 &&
        mainThreadDim.left > 0 &&
        secondThreadDim.left > 0 &&
        mainThreadDim.left < secondThreadDim.left
      ) {
        this.territoryMapsService.setPadding({
          top: pad,
          right: pad,
          bottom: pad,
          left: menuDim.width + mainThreadDim.width + secondThreadDim.width + pad,
        });
        this.lastState = 60;
        return;
      }
