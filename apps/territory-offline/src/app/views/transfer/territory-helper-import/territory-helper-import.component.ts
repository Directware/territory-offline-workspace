    ...
    switch (type) {
      case 'preacher':
        if (file.name.endsWith('.xlsx')) {
          ...
          this.translate
            .get('transfer.import.wrongFileType')
            .pipe(take(1))
      ...
      default:
        if (file.name.endsWith('.json')) {
          myReader.onloadend = () =>
            this.importTerritoryResults(JSON.parse(<string>myReader.result));
          myReader.readAsText(file);
        } else {
          this.translate
            .get('transfer.import.noJsonFile')
            .pipe(take(1))
            .subscribe((translation: string) => alert(translation));
        }
    }
  ...
  private importAssignmentResults(data: any) {
    ...
    workbook.SheetNames.forEach((sheetName) => {
      ...
      for (let rowNum = range.s.r; rowNum <= range.e.r; rowNum++) {
        ...
        const preacher = this.parsedPreacher.filter(
          (p) =>
            preacherName.toUpperCase().includes(p.name.toUpperCase()) &&
            preacherName.toUpperCase().includes(p.firstName.toUpperCase())
        )[0];

        const territory = this.parsedTerritories.filter(
          (t) => t.name === territoryName && t.key === territoryKey
        )[0];
  ...
  private importAddressesResults(data: any) {
    ...
    workbook.SheetNames.forEach((sheetName) => {
      ...
      for (let rowNum = range.s.r; rowNum <= range.e.r; rowNum++) {
        ...
        const territoryName = sheet[XLSX.utils.encode_cell({ r: rowNum, c: 0 })]['w'];
        const territory = this.parsedTerritories.filter(
          (t) => t.key === territoryKey && t.name === territoryName
        )[0];
