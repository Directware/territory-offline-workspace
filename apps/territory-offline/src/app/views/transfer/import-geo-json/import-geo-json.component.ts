...
export class ImportGeoJsonComponent implements OnInit
{
  ...
  public setMapping(prop: string)
  {
    ...
    const toBeChosen = Object.keys(this.keyMappings).filter(key => !this.keyMappings[key].propName).map(key => key);
  ...
  private async showImportDialog(territories, drawings)
  {
    ...
    setTimeout(() => this.router.navigate(["/territories", {outlets: {'second-thread': null}}]), 300);
  }
}
