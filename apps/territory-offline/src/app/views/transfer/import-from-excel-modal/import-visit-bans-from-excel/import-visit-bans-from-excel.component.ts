...
export class ImportVisitBansFromExcelComponent implements OnInit, AfterViewInit {
  ...
  public async ngAfterViewInit() {
    ...
    setTimeout(
      () => (this.visitBanImportMapperStepper = new VisitBanImportMapperStepper(this.matStepper))
    );
  }
  ...
  public shouldHide(step: string, column: ExcelColumn): boolean {
    return (
      this.excelToEntityMapper.isValueAlreadyInUse(column) &&
      this.excelToEntityMapper.getColumnValueOf(step) !== column.value
    );
  }
    ...
    combineLatest([
      ...
      .pipe(
        ...
        tap(([existingVisitBans, territories, drawings]) => {
          const excelToEntityParser = new ExcelToEntityParser(
            this.excelToEntityMapper,
            this.workbook
          );
          let parsedVisitBans = excelToEntityParser.extractVisitBans();
          const locator = new GpsToTerritoryLocator(territories, drawings);

          parsedVisitBans = parsedVisitBans.map((vb) => ({
            ...vb,
            territoryId: locator.locate(vb.gpsPosition),
          }));
  ...
  public overrideExistingVisitBans(
    allExisting: VisitBan[],
    allToBeImported: VisitBan[]
  ): VisitBan[] {
  ...
  private scrollMatStepToTop() {
    setTimeout(() => {
      ...
      const currentStepHeaderElement = this.elementRef.nativeElement.querySelectorAll(
        '.mat-step-header'
      )[currentStep] as HTMLElement;
