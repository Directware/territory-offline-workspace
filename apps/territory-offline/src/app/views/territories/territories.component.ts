...
export class TerritoriesComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.territories$ = this.store.pipe(
      select(selectAllTerritories),
      map((terr) => (!terr || terr.length === 0 ? null : terr))
    );
  ...
  public toggleMapFilter(propName: string) {
    ...
    const result = this.mapsService.setFilterDrawingsByStatus(
      this.mapFilter[propName].status,
      this.mapFilter[propName].show
    );
