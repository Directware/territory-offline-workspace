...
export class PrintTerritoryComponent implements OnInit, OnDestroy {
    ...
    this.store
      .pipe(
        ...
        tap((territory: Territory) =>
          this.territoryMapsService.setPrintingDrawingColor(
            this.printedMapConfiguration['territoryColor'],
            this.printedMapConfiguration['opacity']
          )
        )
      )
      .subscribe();
  ...
  public ngOnDestroy(): void {
    if (this.mapDomElement) {
      ...
      this.renderer2.setStyle(
        this.mapDomElement.querySelector('.mapboxgl-control-container'),
        'display',
        'initial'
      );
  ...
  public async moveForward(currentTerritoryId: string) {
    ...
    const nextTerritory = territories[currentIndex + 1]
      ? territories[currentIndex + 1]
      : territories[0];
  ...
  public saveDrawingPrintAlignment(silent?: boolean) {
    if (this.territory) {
      ...
      if (!silent) {
        this.translate
          .get('territory.print.alignmentSaved')
          .pipe(take(1))
  ...
  private handleBleedingEdges() {
    ...
    const line_horizontal_top_right = this.mapDomElement.querySelector(
      '.line.horizontal.top.right'
    );
    const line_horizontal_bottom_left = this.mapDomElement.querySelector(
      '.line.horizontal.bottom.left'
    );
    const line_horizontal_bottom_right = this.mapDomElement.querySelector(
    ...
    const line_vertical_bottom_left = this.mapDomElement.querySelector(
      '.line.vertical.bottom.left'
    );
    const line_vertical_bottom_right = this.mapDomElement.querySelector(
      '.line.vertical.bottom.right'
    );

    const back_line_horizontal_top_left_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.horizontal.top.left'
    );
    const back_line_horizontal_top_right_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.horizontal.top.right'
    );
    const back_line_horizontal_bottom_left_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.horizontal.bottom.left'
    );
    const back_line_horizontal_bottom_right_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.horizontal.bottom.right'
    );
    const back_line_vertical_top_left_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.vertical.top.left'
    );
    const back_line_vertical_top_right_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.vertical.top.right'
    );
    const back_line_vertical_bottom_left_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.vertical.bottom.left'
    );
    const back_line_vertical_bottom_right_ALL = this.mapDomElement.querySelectorAll(
      'app-print-territory-back .line.vertical.bottom.right'
    );
    ...
    this.renderer2.setStyle(
      line_horizontal_top_left,
      'bottom',
      this.tcf.dimensions.h + bleedThickness + this.tcf.dimensions.dim
    );
    this.renderer2.setStyle(
      line_horizontal_top_left,
      'left',
      bleedLinePosition + this.tcf.dimensions.dim
    );
    this.renderer2.setStyle(
      line_horizontal_top_right,
      'bottom',
      this.tcf.dimensions.h + bleedThickness + this.tcf.dimensions.dim
    );
    this.renderer2.setStyle(
    ...
    this.renderer2.setStyle(
      line_vertical_top_left,
      'right',
      this.tcf.dimensions.w + bleedThickness + this.tcf.dimensions.dim
    );
    this.renderer2.setStyle(
      line_vertical_top_right,
      'top',
      bleedLinePosition + this.tcf.dimensions.dim
    );
    this.renderer2.setStyle(
  ...
  private createMapBack(territory: Territory) {
    ...
    const domElem = (this.mapBackComponentRef.hostView as EmbeddedViewRef<any>)
  ...
  private createMapHeading(territory: Territory) {
    ...
    const domElem = (this.mapHeadingComponentRef.hostView as EmbeddedViewRef<any>)
      .rootNodes[0] as HTMLElement;
  ...
  private getLastPrintingPreferences() {
    this.printedMapConfiguration =
      JSON.parse(localStorage.getItem('printedMapConfiguration')) || {};
    this.tcf = TerritoryCardFormats.filter(
      (format) => format.id === this.printedMapConfiguration.chosenFormatId
    )[0];
