...
export class OverdueAssignmentsComponent implements OnInit, OnDestroy {
        ...
        tap(
          (params) =>
            (this.assignments$ = this.store.pipe(
              ...
              map((assignments) =>
                assignments.sort((dto1, dto2) => (dto1.startTime > dto2.startTime ? 1 : -1))
              ),
              tap((assignments: Assignment[]) => {
                this.store
                  .pipe(
                    ...
                    tap((territories) => {
                      if (assignments) {
                        const tmp = territories.filter((t) =>
                          assignments.map((a) => a.territoryId).includes(t.id)
                        );
                        this.territoryMapsService.focusOnDrawingIds(
                          tmp.map((t) => t.territoryDrawingId)
                        );
                      }
                    })
                  )
  ...
  public async shareList() {
    ...
    const messageHelper = [
      this.translateService.instant('overdueAssigments.shareOverdueFirstSentence') + ' \n',
    ];
    assignments.forEach((a) => {
      const territory = territories.filter((t) => t.id === a.territoryId)[0];
      messageHelper.push(
        `\t * ${territory.key} ${territory.name} seit ${createDurationPhrase(a.startTime)}`
      );
    });
    ...
    this.router.navigate([
      {
        outlets: {
          'second-thread': [
            'assignment',
            this.activatedRoute.snapshot.params.territoryId,
            assignment.id,
          ],
        },
      },
