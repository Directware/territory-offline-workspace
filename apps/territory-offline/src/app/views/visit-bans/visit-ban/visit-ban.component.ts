...
export class VisitBanComponent implements OnInit, OnDestroy {
  ...
  public cancel() {
    ...
    this.router.navigate([
      {
        outlets: {
          'second-thread': ['visit-bans', this.activatedRoute.snapshot.params.territoryId],
        },
      },
    ]);
  }
  ...
  public setVisitBanManually() {
    ...
    this.mapsService.getMap().once('click', async (e) => {
      ...
      .pipe(
        ...
        tap((action) =>
          this.lastDoingsService.createLastDoing(
            lastDoingAction,
            action.visitBan.street + ' ' + action.visitBan.streetSuffix
          )
        ),
        tap(() => this.cancel())
      )
  ...
  private async setVisitBan(
    ...
    if (!territories.length) {
      this.translate
        .get('visitBan.noTerritoryMapped')
        .pipe(take(1))
  ...
  private initFormGroup(vb: VisitBan, territoryId: string) {
    ...
    if (vb && vb.gpsPosition && !this.router.url.includes('all-visit-bans')) {
      ...
      this.mapsService.setMarker(
        vb.id,
        [vb.gpsPosition.lng, vb.gpsPosition.lat],
        `<p>${vb.name || translation}</p>`
      );
    }
    ...
    const applicableTerritories = await combineLatest([
      ...
      .pipe(
        ...
        map(([drawings, territories]) => {
          const applicableDrawings = drawings.filter(
            (d) =>
          ...
          if (applicableDrawings.length) {
            return territories.filter((t) =>
              applicableDrawings.map((d) => d.id).includes(t.territoryDrawingId)
            );
          }
  ...
  private geocode(input: string) {
    ...
    if (!!input && input.length > 3 && hasNumberRegExp.test(input)) {
      this.territoryMapsService.geocode(`${input}`).subscribe(
