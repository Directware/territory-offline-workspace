...
export class PublisherComponent implements OnInit, OnDestroy {
      ...
      .pipe(
        ...
        tap((action) =>
          this.lastDoingsService.createLastDoing(
            lastDoingAction,
            action.publisher.firstName + ' ' + action.publisher.name
          )
        ),
  ...
  public tryToDeletePublisher() {
    this.translate
      .get(['publisher.reallyDelete', 'publisher.canNotDelete'], {
        firstName: this.publisher.get('firstName').value,
        name: this.publisher.get('name').value,
      })
      .pipe(take(1))
      .subscribe((translations: { [key: string]: string }) => {
        ...
        if (canDelete) {
          this.canDeletePublisher().subscribe((canDelete) => {
            if (canDelete) {
    ...
    this.publisherTerritories$ = this.store.pipe(
      ...
      tap((territories) =>
        this.territoryMapsService.focusOnDrawingIds(
          territories.map((dto) => dto.t.territoryDrawingId)
        )
      )
    );
  }
