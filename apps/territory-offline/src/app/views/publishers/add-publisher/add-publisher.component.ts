...
export class AddPublisherComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.allPublishers$ = this.store.pipe(
      select(selectPublishers),
      tap((publishers) => this.setInitialPublisher(publishers))
    );

    this.searchPublisherControl.valueChanges
      .pipe(
        ...
        tap(([value, publishers]: [string, Publisher[]]) => {
          if (value && value.length > 0) {
            this.searchResults = publishers.filter(
              (p) =>
  ...
  public addPublisher(publisher: Publisher) {
    ...
    this.searchResults = [];
    this.searchPublisherControl.setValue(`${publisher.name} ${publisher.firstName}`, {
      emitEvent: false,
    });
  }
