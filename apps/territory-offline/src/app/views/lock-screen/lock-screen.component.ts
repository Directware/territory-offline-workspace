...
} from '../../core/store/last-doings/last-doings.actions';
...
export class LockScreenComponent implements OnInit {
  ...
  public tryToUnLockWithPassword() {
    if (this.passwordNeeded && this.appPassword && !this.coolDown) {
      ...
      this.cryptoService.isPasswordCorrect(this.appPassword).subscribe((isCorrect) => {
        if (isCorrect) {
    ...
    if (!!password) {
      this.store
        .pipe(
          ...
          first(),
          map((settings) =>
            this.cryptoService.decryptSecretKey(
              password,
              settings.publicKey,
  ...
  private loadAllDataListeners(showEncryptionProgress?: boolean) {
    ...
    this.actions$
      .pipe(
        ...
        take(1),
        concatMap((action) =>
          of(action).pipe(withLatestFrom(this.store.pipe(select(selectSettings))))
        ),
        tap(([action, settings]) =>
          this.store.dispatch(UseCongregation({ congregationId: settings.currentCongregationId }))
        ),
