...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.overdueAssignments$ = this.store.pipe(
      ...
      map((dtos: any[]) => {
        dtos.forEach((dto) =>
          dto.assignments.sort((dto1, dto2) => (dto1.startTime > dto2.startTime ? 1 : -1))
        );
        return dtos.sort((dto1, dto2) =>
          dto1.assignments[0].startTime > dto2.assignments[0].startTime ? 1 : -1
        );
      })
    );
    this.overdueTerritories$ = this.store.pipe(
      select(selectOverdueTerritories),
      map((dtos) =>
        dtos.sort((dto1, dto2) =>
          dto1.lastAssignmentEndTime > dto2.lastAssignmentEndTime ? 1 : -1
        )
      )
    );
    ...
    this.router.events
      .pipe(
        ...
        map(
          () =>
            this.activatedRoute.snapshot.root.children.filter(
              (tmp) => tmp.outlet === 'second-thread'
            )[0]
        ),
        tap(
          (child) =>
            (this.currentSecondThreadUuid =
              child && child.params
                ? child.params.id || child.params.publisherId || child.params.visitBanId
                : null)
        )
      )
      .subscribe();
  ...
  public showPublisherOverdueAssignments(dto) {
    if (this.clickOnSameSecondThreadUuid(dto.publisher.id)) {
      ...
      this.router.navigate([
        { outlets: { 'second-thread': ['overdue-assignments', dto.publisher.id] } },
      ]);
    }
  }
  ...
  private clickOnSameSecondThreadUuid(id: string): boolean {
    if (
      !this.activatedRoute.snapshot.root.children.filter((tmp) => tmp.outlet === 'second-thread')[0]
    ) {
      return false;
    }
