...
export class AppModule {
  constructor(
    ...
    AVAILABLE_LANGUAGES.forEach((lang) =>
      this.translateService.setTranslation(lang.key, lang.translations)
    );
      ...
      .pipe(
        ...
        tap(async ({ settings }) => {
          ...
          if (settings && settings.initialConfigurationDone) {
            const translationsExists =
              settings?.appLanguage &&
              this.translateService
                .getLangs()
                .includes(settings.appLanguage.languageCode);
            if (
              translationsExists &&
              settings.appLanguage &&
            ...
            let systemLang = this.languageService.getLanguageByCode(
              langCode.value
            );

            if (
              systemLang &&
              this.translateService.getLangs().includes(systemLang.languageCode)
            ) {
