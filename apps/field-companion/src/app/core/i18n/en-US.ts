export const enUS = {
  title: "Field Companion",
  common: {
    done: "Done",
    info: "Info",
    comment: "Comment",
    preview: "Preview",
    leave: "Leave",
    soon: "Available soon",
    newFeature: "New Feature!",
    cool: "Cool!"
  },
  featureConfirmation: {
    reportUpToTheMinute: {
      title: "Minute-based reporting",
      description: "Press and hold the hour recording button to report your time to the minute. You can now also specify in the settings the steps you want to take to record your time."
    }
  },
  actions: {
    continue: "Continue",
    send: "Send"
  },
  welcome: {
    origin: "by Territory Offline",
    feature1: "Report your service over the month",
    feature2: "Set hour targets and track your progress",
    feature3: "Send your report at the end of the month",
    feature4: "Receive areas and view them on the map"
  },
  service: {
    noDayChosen: "No day selected",
    noDayChosenDesc: "Select a day to edit or add reports",
    report: "Report",
    duration: "Hours",
    studies: "Bible studies",
    deliveries: "Placements",
    videos: "Videos",
    returnVisits: "RV",
    chooseMonth: "Select month",
    inputStudies: "Specify Bible Studies",
    durationGoals: "Define target hours",
    sendReport: "Send report",
    editReport: "Edit report",
    months: "Months",
    month: "Month",
    day: "Day",
    minutes: "Minutes",
    monthly: "Monthly",
    year: "Year",
    yearly: "Yearly",
    goals: "Hourly goals",
    goalsDesc: "You can set a monthly and annual hourly target. The app shows you your progress.",
    goalsInfo: "Tap on the number of hours in the report overview to switch between the display of the monthly / annual target.",
    studiesDesc: "Enter the number of different people with whom you study the Bible so that it appears in your report.",
    studiesInfo: "In the Overview report, tap the study icon (Person) to change the number of studies.",
    explainDurationTrimmer: "Your reported hours are rounded off and the remaining time is carried over to the next month."
  },
  week: {
    shortNames: {
      monday: "Mo",
      tuesday: "Tu",
      wednesday: "We",
      thursday: "Th",
      friday: "Fr",
      saturday: "Sa",
      sunday: "Su",
    }
  },
  month: {
    "0": "January",
    "1": "February",
    "2": "March",
    "3": "April",
    "4": "May",
    "5": "June",
    "6": "July",
    "7": "August",
    "8": "September",
    "9": "October",
    "10": "November",
    "11": "December",
  },
  territories: {
    title: "Territories",
    noTerritories: "No territories",
    noTerritoriesDesc: "This feature is not yet enabled.",
    yourTerritories: "Your Territories",
    feature1: "Receive areas via link from your territory servant",
    feature2: "View your territory on detailed maps",
    feature3: "Track your location and keep track of where you've been",
  },
  settings: {
    title: "Settings",
    yourName: "Your name",
    yourId: "User ID",
    yourLanguage: "Your language",
    changeLanguage: "Change language",
    language: "Language",
    share1: "You like our app?",
    share2: "Share it with others!",
    aboutApp: "About the app",
    appIdea: "Idea & Developer",
    appDesign: "Design & Text",
    monthlyReminder: "Reminder for report submission",
    noBackupFile: "It is not a backup file.",
    importSucceed: "Import was successful!",
    changeDurationStep: "Steps in time recording",
    durationStep: "Step in minutes",
    ldc: "LDC Duration",
    general: "General",
    report: "Report",
    data: "Data",
    app: "App",

    actions: {
      grapesImport: "Import data from Grapes",
      donate: "Support developers",
      bugReport: "Bug report",
      territoryOffline: "Territory Offline site",
      about: "About this App",
      grapesImportSucceed: "All right. The import was successful!",
      resetApp: "Delete all data",
      reallyReset: "Do you really want to reset the app?",
      sendYourId: "Share your User ID",
      exportReports: "Export to excel",
      exportBackup: "Create backup",
      importBackup: "Import backup"
    }
  },
  localNotifications: {
    monthlyReminderTitle: "Report",
    monthlyReminderBody: "It's that time again! Your report is waiting to be sent 📮"
  }
};
