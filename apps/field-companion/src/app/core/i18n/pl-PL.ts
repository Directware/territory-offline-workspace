export const plPL = {
  title: "Field Companion",
  common: {
    done: "Gotowe",
    info: "Info",
    comment: "Komentarz",
    preview: "Podgląd",
    leave: "Wyjście",
    soon: "Wkrótce dostępne",
    newFeature: "Nowa funkcja!",
    cool: "Cool!"
  },
  featureConfirmation: {
    reportUpToTheMinute: {
      title: "Sprawozdawczość minutowa",
      description: "Naciśnij i przytrzymaj przycisk nagrywania godzin, aby zgłosić swój czas do minuty. Teraz możesz również określić w ustawieniach kroki, które chcesz podjąć, aby nagrać swój czas."
    }
  },
  actions: {
    continue: "Kontynuuj",
    send: "Prześlij"
  },
  welcome: {
    origin: "by Territory Offline",
    feature1: "Sprawozdania za miesiąc z twojej służby",
    feature2: "Wyznaczaj cele godzinowe i śledź swoje postępy",
    feature3: "Wyślij swój owoc na koniec miesiąca",
    feature4: "Odbieraj tereny i przeglądaj je na mapie"
  },
  service: {
    noDayChosen: "Nie wybrano żadnego dnia",
    noDayChosenDesc: "Wybierz dzien do edycji lub dodawania raportów",
    report: "Raport",
    duration: "Godziny",
    studies: "Studia",
    deliveries: "Oddania",
    videos: "Wideo",
    returnVisits: "Odwiedziny",
    chooseMonth: "Wybierz miesiąc",
    inputStudies: "Podaj studia biblijne",
    durationGoals: "Cel godzinowy",
    sendReport: "Wyślij raport",
    editReport: "Edytuj raport",
    months: "Miesiące",
    month: "Miesiąc",
    day: "Dzień",
    minutes: "Minuty",
    monthly: "Miesięcznie",
    year: "Rok",
    yearly: "Rocznie",
    goals: "Cele godzinowe",
    goalsDesc: "Ustaw miesięczny i roczny cel godzinowy. Aplikacja pokazuje twoje postępy.",
    goalsInfo: "Dotknij liczbę godzin w podsumowaniu raportu, aby przełączyć się pomiędzy wyświetlaniem celu miesięcznego i rocznego.",
    studiesDesc: "Podaj liczbę różnych osób, z którymi studiujesz Biblię, tak aby pojawiła się ona w twoim raporcie.",
    studiesInfo: "W podsumowaniu raportu dotknij ikony studium biblijnego (Osoba), aby zmienić liczbę studiów.",
    explainDurationTrimmer: "Twoje godziny zostaną zaokrąglone, a pozostały czas zostanie przeniesiony na następny miesiąc."
  },
  week: {
    shortNames: {
      monday: "Po",
      tuesday: "Wt",
      wednesday: "Śr",
      thursday: "Cz",
      friday: "Pi",
      saturday: "So",
      sunday: "Ni",
    }
  },
  month: {
    "0": "Styczeń",
    "1": "Luty",
    "2": "Marzec",
    "3": "Kwiecień",
    "4": "Maj",
    "5": "Czerwiec",
    "6": "Lipiec",
    "7": "Sierpień",
    "8": "Wrzesień",
    "9": "Październik",
    "10": "Listopad",
    "11": "Grudzień",
  },
  territories: {
    title: "Tereny",
    noTerritories: "Brak terenow",
    noTerritoriesDesc: "Ta funkcja nie jest jeszcze włączona.",
    yourTerritories: "Twoje tereny",
    feature1: "Odbieraj tereny od swojego sługi terenow",
    feature2: "Zobacz swój teren na szczegółowych mapach",
    feature3: "Śledź swoją lokalizację i śledź, gdzie byłeś",
  },
  settings: {
    title: "Ustawienia",
    yourName: "Twoje imię",
    yourId: "User ID",
    yourLanguage: "Twój język",
    changeLanguage: "Zmień język",
    language: "Język",
    share1: "Podoba ci się ta aplikacja?",
    share2: "Podziel się nią z innymi!",
    aboutApp: "O aplikacji",
    appIdea: "Idea & realizacja",
    appDesign: "Design & Tekst",
    monthlyReminder: "Przypomnienie o owocu",
    noBackupFile: "Plik nie jest typem kopii zapasowej",
    importSucceed: "Import zakończył się sukcesem!",
    changeDurationStep: "Etapy zapisu czasu",
    durationStep: "Krok zapisu czasu w minutach",
    ldc: "LDC Godziny",
    general: "Ogólne",
    report: "Owoc",
    data: "Dane",
    app: "App",

    actions: {
      grapesImport: "Importowanie danych z Grapes",
      donate: "Wspieraj deweloperów",
      bugReport: "Zgłoś problem",
      territoryOffline: "Strona Territory Offline",
      about: "O tej aplikacji",
      grapesImportSucceed: "Dobrze. Import był udany!",
      resetApp: "Zresetuj aplikację",
      reallyReset: "Czy naprawdę chcesz zresetować aplikację?",
      sendYourId: "Udostępnij swój identyfikator użytkownika",
      exportReports: "Eksport do Excel",
      exportBackup: "Tworzenie kopii zapasowej",
      importBackup: "Importowanie kopii zapasowej"
    }
  },
  localNotifications: {
    monthlyReminderTitle: "Owoc",
    monthlyReminderBody: "Znów nadszedł czas! Twój owoc czeka na wysłanie 📮"
  }
};
