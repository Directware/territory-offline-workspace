...
export class MapService {
  ...
  public async initWithAllTerritories() {
    const territoryCards = await this.store
      .pipe(select(selectAllTerritoryCards), first())
      .toPromise();
    const mergedDrawings = mergeDrawings(
      territoryCards.map((tc) => tc.drawing)
    );
    const center = Turf.center(mergedDrawings.featureCollection).geometry
      .coordinates;
  ...
  public async initWithOneTerritory(territoryId: string) {
    const territoryCard = await this.store
      .pipe(select(selectTerritoryCardById, territoryId), first())
      .toPromise();
  ...
  public showUserMarker(coordinates) {
    if (!this.userMarker) {
      ...
      this.userMarker = new MapBox.Marker(mapMarker)
        .setLngLat(coordinates)
        .addTo(this._map);
  ...
  private init(center, mergedDrawings) {
    Network.getStatus().then((status) => {
      if (status.connected) {
        ...
        this._map.on("load", () => {
          this._map.addSource(this.mapSourceName, {
            type: "geojson",
            data: mergedDrawings.featureCollection,
          });
          this.initLayers();
          this._map.fitBounds(Turf.bbox(mergedDrawings.featureCollection), {
            padding: 30,
          });
        });
