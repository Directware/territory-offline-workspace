...
import {UpsertTerritoryCard, UpsertTerritoryCardSuccess} from "../store/territory-card/territory-card.actions";
...
export class TerritoryCardService
{
  ...
  private navigateToTerritoryCardAfterAdding()
  {
    this.actions$.pipe(
      ...
      first(),
      tap((action) => this.ngZone.run(() => setTimeout(() => this.router.navigate([`/territory/${action.territoryCard.id}`]), 300)))
    ).subscribe();
  }
}
