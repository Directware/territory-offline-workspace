...
export class DataExportService {
  ...
  public async getFileName(): Promise<string> {
    ...
    return `field-companion backup ${today.getFullYear()}-${
      today.getMonth() + 1
    }-${today.getDate()} ${today.getHours()}-${minutes}`;
  }

  public async giveBackTerritory(territoryCard: TerritoryCard) {
    const today = new Date();
    await FileSharer.share({
      filename: `${territoryCard.territory.key}-${
        territoryCard.territory.name
      }-${today.getDate()}-${today.getMonth() + 1}-${today.getFullYear()}.${
        FileExtensions.TERRITORY
      }`,
      base64Data: btoa(
        Pako.gzip(JSON.stringify(territoryCard), { to: "string" })
      ),
      contentType: "text/plain;charset=utf-8",
      android: {
        chooserTitle: `${this.translateService.instant(
          "territories.giveBack"
  ...
  private async exportAll() {
    ...
    const gzippedData = Pako.gzip(JSON.stringify(allDataDictionary), {
      to: "string",
    });
