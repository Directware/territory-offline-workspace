...
export class LocalNotificationsService {
  private localStorageKey =
    "[LocalNotificationsService] last monthlyReportReminder";
  ...
  public async areLocalNotificationsEnabled() {
    const settings = await this.store
      .pipe(select(selectSettings), take(1))
      .toPromise();
        ...
        notifications: [
          {
            title: this.translateService.instant(
              "localNotifications.monthlyReminderTitle"
            ),
            body: this.translateService.instant(
              "localNotifications.monthlyReminderBody"
            ),
            id: 1,
