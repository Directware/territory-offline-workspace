...
export class DailyReportsEffects {
  private loadDailyReports$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((dailyReports: DailyReport[]) => LoadDailyReportsSuccess({ dailyReports }))
    )
  );
  ...
  private upsertDailyReport$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((promise: Promise<TimedEntity>) => from(promise)),
      map((dailyReport: DailyReport) => UpsertDailyReportSuccess({ dailyReport }))
    )
  );
  ...
  private bulkImportDailyReports$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((promise: Promise<TimedEntity[]>) => from(promise)),
      map((dailyReports: DailyReport[]) => BulkImportDailyReportsSuccess({ dailyReports }))
    )
  );
  ...
  private deleteDailyReport$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((promise: Promise<TimedEntity>) => from(promise)),
      map((dailyReport: DailyReport) => DeleteDailyReportSuccess({ dailyReport }))
    )
  );
  ...
  private setStudies$ = createEffect(() =>
    this.actions$.pipe(
      ofType(SetStudies),
      concatMap((action) =>
        of(action).pipe(withLatestFrom(this.store.pipe(select(selectDailyReportForStudy))))
      ),
      map(([action, dailyReport]) =>
        UpsertDailyReport({
