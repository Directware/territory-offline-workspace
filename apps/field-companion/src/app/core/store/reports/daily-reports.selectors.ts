...
import {
  emptyDailyReport,
  emptyMergedDailyReport,
} from "@territory-offline-workspace/shared-interfaces";
import { getServiceYearTimes } from "@territory-offline-workspace/shared-utils";

export const selectDailyReportsFeature = (state: ApplicationState) =>
  state.dailyReports;
...
export const selectNextMonthsDailyReport = createSelector(
  ...
  (dailyReports, state) => {
    return (
      ...
      emptyDailyReport(state.chosenTime.year, state.chosenTime.month + 1, 1)
    );
  }
);
...
export const selectCurrentDailyReport = createSelector(
  ...
  (dailyReports, state) => {
    ...
    return (
      ...
      emptyDailyReport(
        state.chosenTime.year,
        state.chosenTime.month,
        state.chosenCalendarCell.dayIndex
      )
    );
  }
);
...
export const selectCurrentLastDayInMonthDailyReport = createSelector(
  ...
  (dailyReports, state) => {
    const lastDayInMonth = new Date(
      state.chosenTime.year,
      state.chosenTime.month + 1,
      0
    );
    const lastMonthlyDailyReport = dailyReports.filter(
      (dr) =>
    ...
    return (
      lastMonthlyDailyReport ||
      emptyDailyReport(state.chosenTime.year, state.chosenTime.month + 1, 0)
    );
  }
);
...
export const selectDailyReportForStudy = createSelector(
  ...
  (allCurrentDailyReports, state) => {
    const firstReportInTheMonth = allCurrentDailyReports.filter(
      (dr) => dr.creationTime.getDate() === 1
    )[0];
...
export const durationProgress = createSelector(
  ...
  (settings, drState, dailyReports) => {
    const monthlyDailyReports = dailyReports.filter(
      (dr) =>
        dr.creationTime.getFullYear() === drState.chosenTime.year &&
        dr.creationTime.getMonth() === drState.chosenTime.month
    );

    const monthlySum = monthlyDailyReports
      .map((dr) => dr.duration)
      .reduce((all, item) => (all = all + item), 0);

    /* Vom September des Vorjahres bis August des aktuellen Jahres */
    const result = getServiceYearTimes(new Date());
    const yearlyDailyReports = dailyReports.filter(
      (r) =>
        r.creationTime.getTime() >= result.serviceYearStart &&
        r.creationTime.getTime() <= result.serviceYearEnd
    );

    const yearlySum = yearlyDailyReports
      .map((dr) => dr.duration)
      .reduce((all, item) => (all = all + item), 0);
