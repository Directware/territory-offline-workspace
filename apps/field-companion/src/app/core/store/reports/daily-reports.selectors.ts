...
export const selectAllDailyReports = createSelector(selectAll, (reports) => reports);
...
export const durationProgress = createSelector(
  ...
  (settings, drState, dailyReports) => {
    ...
    const yearlyDailyReports = dailyReports.filter(
      (dr) =>
        (dr.creationTime.getFullYear() - 1 === drState.chosenTime.year &&
          dr.creationTime.getMonth() > 7) ||
        (dr.creationTime.getFullYear() === drState.chosenTime.year &&
          dr.creationTime.getMonth() < 8)
    );
