...
const dailyReportsReducer = createReducer(
  ...
  on(LoadDailyReportsSuccess, (state, action) => dailyReportsAdapter.addAll(action.dailyReports, state)),
  on(UpsertDailyReportSuccess, (state, action) => dailyReportsAdapter.upsertOne(action.dailyReport, state)),
  on(BulkImportDailyReportsSuccess, (state, action) => dailyReportsAdapter.upsertMany(action.dailyReports, state)),
  on(DeleteDailyReportSuccess, (state, action) => dailyReportsAdapter.removeOne(action.dailyReport.id, state)),
  on(ChangeChosenTime, (state, action) => ({...state, chosenCalendarCell: null, chosenTime: action.chosenTime}))
);
