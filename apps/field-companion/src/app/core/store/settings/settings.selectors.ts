...
export const selectGoals = createSelector(selectSettingsFeature, (settings) => ({
  yearly: settings.yearlyGoal || 0,
  monthly: settings.monthlyGoal || 0,
}));

export const selectUserLanguage = createSelector(selectSettingsFeature, (settings) =>
  settings && settings.userLanguage ? settings.userLanguage.languageCode : null
);
