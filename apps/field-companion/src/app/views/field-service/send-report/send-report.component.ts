...
export class SendReportComponent implements OnInit, OnDestroy
{
  ...
  public async send()
  {
    combineLatest([this.monthlyReport$, this.chosenTime$])
      .pipe(
        take(1),
        tap(async ([mergedReports, chosenTime]: [MergedDailyReport, CalendarDatasource]) =>
        {
          const isWholeNumber = mergedReports.duration === (Math.floor(mergedReports.duration / 60) * 60);
          let reports = mergedReports;

          if (!isWholeNumber)
          {
            const wholeNumberDuration = Math.floor(mergedReports.duration / 60) * 60;
            const remainingTime = mergedReports.duration - wholeNumberDuration;
          ...
          let shareRet = await Share.share({
            ...
            text: this.evaluateShareTitle(chosenTime) + "\n\n" + this.evaluateShareText(reports),
          });
          return shareRet;
        })
      ).subscribe();
  ...
  {
    return `${this.translateService.instant("service.report")} - ${this.translateService.instant("month." + chosenTime.month)} ${chosenTime.year}:`;
  }

  private evaluateShareText(mergedReports: MergedDailyReport)
  {
    return `${this.translateService.instant("service.duration")}: ${mergedReports.duration / 60}h, `
      + `${this.translateService.instant("service.videos")}: ${mergedReports.videos}, `
      + `${this.translateService.instant("service.deliveries")}: ${mergedReports.deliveries}, `
      + `${this.translateService.instant("service.returnVisits")}: ${mergedReports.returnVisits}, `
      + `${this.translateService.instant("service.studies")}: ${mergedReports.studies}.`;
  }
  ...
  private decreaseReport(remainingMinutes: number)
  {
    this.store.pipe(
      ...
      tap((dailyReports: DailyReport[]) =>
      {
        ...
        {
          const isWholeNumber = dailyReports[i].duration === (Math.floor(dailyReports[i].duration / 60) * 60)

          if (!isWholeNumber && dailyReports[i].duration >= remainingMinutes)
          {
  ...
  private increaseNextMonthsReport(remainingMinutes: number)
  {
    this.store.pipe(
      ...
      take(1),
      tap((dailyReport) => this.store.dispatch(UpsertDailyReport({
        dailyReport: {
          ...dailyReport,
