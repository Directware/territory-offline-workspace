...
export class SendReportComponent implements OnInit, OnDestroy {
    ...
    combineLatest([this.monthlyReport$, this.chosenTime$])
      .pipe(
        ...
        tap(async ([mergedReports, chosenTime]: [MergedDailyReport, CalendarDatasource]) => {
          const isWholeNumber =
            mergedReports.duration === Math.floor(mergedReports.duration / 60) * 60;
  ...
  private evaluateShareTitle(chosenTime: CalendarDatasource) {
    return `${this.translateService.instant('service.report')} - ${this.translateService.instant(
  ...
  private evaluateShareText(mergedReports: MergedDailyReport) {
    return (
      `${this.translateService.instant('service.duration')}: ${mergedReports.duration / 60}h, ` +
      ...
      .pipe(
        ...
        tap((dailyReports: DailyReport[]) => {
          ...
          for (let i = 0; i < dailyReports.length; i++) {
            const isWholeNumber =
              dailyReports[i].duration === Math.floor(dailyReports[i].duration / 60) * 60;
