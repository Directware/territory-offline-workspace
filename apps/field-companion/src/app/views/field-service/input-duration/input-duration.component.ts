...
export class InputDurationComponent implements OnInit, AfterViewInit, OnDestroy
{
  ...
  public ngAfterViewInit()
  {
    ...
    if (originDurationInputElement)
    {
      ...
      const durationBoundingClientRect = this.durationPickerWrapper.nativeElement.getBoundingClientRect();

      this.durationPickerWrapper.nativeElement.style.left = boundingClientRect.left + "px";
      this.durationPickerWrapper.nativeElement.style.top = (boundingClientRect.top - durationBoundingClientRect.height - 8) + "px";
