...
{
  private readonly LANGUAGES_DICTIONARY: { [languageCode: string]: { name: string, nativeName: string } };
  ...
  constructor()
  {
    ...
    Object.keys(this.LANGUAGES_DICTIONARY).forEach(langCode => this.LANGUAGES.push({
  ...
  public searchLanguage(searchPhrase: string): ToLanguage[]
  {
    if (searchPhrase)
    {
      ...
      return this.LANGUAGES.filter(lang => lang.name.toLowerCase().startsWith(normalizedSearchPhrase) || lang.nativeName.toLowerCase().startsWith(normalizedSearchPhrase));
    }
  }
