...
export function createDurationPhrase(startDate: Date) {
  if (typeof startDate !== 'string' && startDate && startDate.getTime() > 0) {
    ...
    remainingDuration =
      months > 0 ? remainingDuration - months * averageDaysInMonth : remainingDuration;
  ...
  if (territory) {
    return {
      ...
      durationPhrase: `${territory.key} (${
        assignment
          ? createDurationPhrase(isAssigned ? assignment.startTime : assignment.endTime)
          : '-'
      })`,
    };
  }
...
export function deserializeObject(objectAsString: string): any {
  if (!objectAsString || objectAsString === 'null' || objectAsString === 'NULL') {
