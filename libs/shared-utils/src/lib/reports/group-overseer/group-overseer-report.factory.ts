...
export function groupOverseerFactory(
  tags: Tag[],
      ...
      publishers: publishers
        ...
        .map((p) => ({
          ...
          territories: assignments
            .filter((a) => a.publisherId === p.id)
            .map((a) => combineAssignmentWithTerritory(a, territories)),
        })),
    })
  );
...
export function groupOverseerPdfMakeContentFactory(
  groupOverseerReport: GroupOverseerReport,
  translations: { since: string; noTerritory: string }
) {
  const content = [];
